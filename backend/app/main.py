...
def custom_openapi():
    ...
    app.openapi_schema = get_openapi(
        title="Argus Crypto API", version="1.0.0", routes=app.routes
    )

    app.openapi_schema["paths"]["/v1/tickers/{ticker_id}/prices/ws"] = {
        "get": {
            "tags": ["Prices"],
            "summary": "Real-time Prices By Ticker Id - last 10 minutes by default (websocket, do not try as endpoint)",
