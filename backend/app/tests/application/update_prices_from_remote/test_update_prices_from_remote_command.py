...
class TestUpdatePricesFromRemoteCommand(TestCase):
    def setUp(self) -> None:
        ...
        self.command = UpdatePricesFromRemoteCommand(
            self.exchange_client, self.exchange_repository, self.price_repository
        )

    def test_execute(self) -> None:
        tickers = [Ticker(), Ticker()]
        self.exchange_repository.find_exchange.return_value = Exchange(tickers=tickers)
        ...
        self.exchange_repository.find_exchange.assert_called_once_with(
            fetch_tickers=True
        )
